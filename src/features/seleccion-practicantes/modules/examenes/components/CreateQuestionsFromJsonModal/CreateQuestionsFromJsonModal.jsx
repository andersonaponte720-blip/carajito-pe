...
import { createQuestionsBatch } from '../../services'
import { useToast } from '@shared/components/Toast'
import styles from './CreateQuestionsFromJsonModal.module.css'
...
export function CreateQuestionsFromJsonModal({ isOpen, onClose, onSuccess, examId }) {
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (event) => {
        try {
          ...
          const cleanedData = validateJSON(JSON.stringify(parsed))
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await createQuestionsBatch(examId, parsed)

      toast.success('Preguntas creadas exitosamente')
      handleClose()
      if (onSuccess) {
        onSuccess(response)
      }
    ...
    <div className={styles.overlay} onClick={handleClose}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        <div className={styles.header}>
          <h2 className={styles.title}>Crear Preguntas desde JSON</h2>
        ...
        <div className={styles.footer}>
          ...
          >
            ...
            {isSubmitting ? 'Creando...' : 'Crear Preguntas'}
          </button>
        </div>
      </div>
    </div>
